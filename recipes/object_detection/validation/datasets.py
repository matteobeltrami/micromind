...
from ultralytics.utils import colorstr, is_dir_writeable
...
class BaseDataset(Dataset):
    ...
    Args:
        ...
        cache (bool, optional): Cache images to RAM or disk during training.
            Defaults to False.
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                try:
                    ...
                    LOGGER.warning(
                        f"{self.prefix}WARNING ⚠️ Removing corrupt *.npy image \
                        file {fn} due to: {e}"
                    )
...
class YOLODataset(BaseDataset):
    ...
    Returns:
        (torch.utils.data.Dataset): A PyTorch dataset object that can be used for
            training an object detection model.
    """

    def __init__(self, *args, data=None, task="detect", transformations=None, **kwargs):
        """
        Initializes the YOLODataset with optional configurations for segments
        and keypoints.
        """
    ...
    def cache_labels(self, path=Path("./labels.cache")):
        ...
        if self.use_keypoints and (nkpt <= 0 or ndim not in {2, 3}):
            raise ValueError(
                "'kpt_shape' in data.yaml missing or incorrect. Should be a list "
                "with [number of keypoints, number of dims (2 for x,y or 3 for "
                "x,y,visible)], i.e. 'kpt_shape: [17, 3]'"
            )
        with ThreadPool(NUM_THREADS) as pool:
            results = pool.imap(
    ...
    def get_labels(self):
        ...
        if not labels:
            LOGGER.warning(
                f"WARNING ⚠️ No images found in {cache_path}, training may not work \
                correctly. {HELP_URL}"
            )
        ...
        if len_segments and len_boxes != len_segments:
            LOGGER.warning(
                f"WARNING ⚠️ Box and segment counts should be equal, but got \
                len(segments) = {len_segments}, "
                f"len(boxes) = {len_boxes}. To resolve this only boxes will be used \
                and all segments will be removed. "
                "To avoid this please supply either a detect or segment dataset, not \
        ...
        if len_cls == 0:
            LOGGER.warning(
                f"WARNING ⚠️ No labels found in {cache_path}, \
                training may not work correctly. {HELP_URL}"
            )
        return labels
    ...
    def build_transforms(self, hyp=None):
        ...
        if self.augment:
            ...
            transforms = Compose(
                [LetterBox(new_shape=(self.imgsz, self.imgsz)), self.transformations]
            )
        transforms.append(
            Format(
                bbox_format="xywh",
    ...
    def update_labels_info(self, label):
        ...
        Note:
            cls is not with bboxes now, classification and semantic segmentation need an
            independent cls label. Can also support classification and semantic
            segmentation by adding or removing dict keys there.
        """
...
def build_yolo_dataset(
    ...
    """Build YOLO Dataset."""
    dataset = None if multi_modal else YOLODataset
    return dataset(
        img_path=img_path,
        imgsz=cfg.imgsz,
        batch_size=batch,
